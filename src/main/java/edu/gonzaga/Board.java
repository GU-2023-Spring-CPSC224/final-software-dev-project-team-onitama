...
public class Board {
  private String[][] boardStartValues = {
    {"b", "0", "0", "0", "r"},
    {"b", "0", "0", "0", "r"},
    {"B", "0", "0", "0", "R"},
    {"b", "0", "0", "0", "r"},
  ...
  public boolean checkValidMove(Coordinate pieceCoord, Coordinate destCoord) {
    ...
    // no movement
    if (pieceCoord == destCoord) return false;
    // out of bounds
    if (x1 >= size || x1 < 0) return false;
    if (y1 >= size || y1 < 0) return false;
    if (x2 >= size || x2 < 0) return false;
    if (y2 >= size || y2 < 0) return false;
    // same team
    if (board[x1][y1].getPiece() == board[x2][y2].getPiece()) return false;
    if (board[x1][y1].getPlayer() == board[x2][y2].getPlayer()) return false;
    return true;
  }
  ...
  public void generateDestinations() {
    ...
    if (board[x1][y1].getPlayer() == 2) {
      for (int i = 0; i < curCard.getInvMoves().size(); i++) {
        Coordinate temp =
            new Coordinate(
                x1 + curCard.getMoves().get(i).getX(), y1 + curCard.getMoves().get(i).getY());
      ...
      for (int i = 0; i < curCard.getMoves().size(); i++) {
        Coordinate temp =
            new Coordinate(
                x1 + curCard.getInvMoves().get(i).getX(), y1 + curCard.getInvMoves().get(i).getY());
        destinations.add(temp);
      }
    } else {
  ...
  public void chooseDestination(int choice) {
    if (destinations.isEmpty() != true) makeMove(destinations.get(choice));
  }
  ...
  */
  public void boardButtonPressed(Coordinate cord) {
    if (isPiece(cord) && getPlayer(cord) != lastPlayer) { // if the button press is a peice
      ...
      } else if (getSquare(cord).getPlayer()
          == getSquare(curPiece)
              .getPlayer()) { // if there already is a current piece only change if a piece from the
        // same team is selected
        setCurPiece(cord);
        for (int i = 0; i < destinations.size(); i++) {
    ...
    } else if (!isPiece(cord)
        || getPlayer(cord)
            == lastPlayer) { // if the square is not a piece and if possible moves have been
      // generated move current peice to the button press (need to allow
      // captures still)
    ...
    if (curCard != null
        && curPiece
            != null) { // if there is a card and a piece selected generate destinations and set the
      // squares at those locations to possible
